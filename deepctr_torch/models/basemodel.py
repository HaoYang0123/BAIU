...
class BaseModel(nn.Module):
    def __init__(self, linear_feature_columns, dnn_feature_columns, l2_reg_linear=1e-5, l2_reg_embedding=1e-5,
                 init_std=0.0001, seed=1024, task='binary', device='cpu', gpus=None, token_features=[]):
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
        ...
        if isinstance(x, dict):
            x = [x[feature] for feature in list(self.feature_index.keys())+self.token_features]
        ...
        print("XXX shape:", xxx.shape, torch.from_numpy(y).shape)
            ...
            try:
                with tqdm(enumerate(train_loader), disable=verbose != 1) as t:
                    ...
                    for _, (x_train, y_train) in t:
                        train_sample_idx += 1
                        if train_sample_idx % 100 == 0:
                            print("Process", train_sample_idx, len(train_loader))
