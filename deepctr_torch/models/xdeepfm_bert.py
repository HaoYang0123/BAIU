...
class xDeepFM(BaseModel):
    ...
    def __init__(self, linear_feature_columns, dnn_feature_columns, dnn_hidden_units=(256, 256),
                 ...
                 nlp_dim=32, word_size=31000, word_dim=32, tokenizer=None, title_npy_folder="", query_npy_folder="", des_npy_folder="", userid2ad_list={}):
    ...
    def _get_nlp_feat(self, word_id, word_msk):
        ...
        score = word_fea * word_msk.unsqueeze(-1)  # bs, #words, 32
        fz = torch.sum(score, dim=1)  # bs, 32
        fm = torch.sum(word_msk, dim=1) + 1e-9
        return fz / fm.unsqueeze(-1)  # bs, 32

    def forward(self, X_linear_feat, tit_feat, qry_feat, des_feat, tit_tok_fea, tit_msk_fea, qry_tok_fea, qry_msk_fea, des_tok_fea, des_msk_fea, s_tit_fea, s_des_fea, s_qry_fea, s_tit_des_msk, s_itm_msk, s_tit_tok, s_tit_msk, s_des_tok, s_des_msk, s_qry_tok, s_qry_msk):
        bs = tit_feat.shape[0]
        #print("BS", bs)
        ...
        nlp_bert_feat = torch.cat([tit_feat, qry_feat, des_feat, s_att_feat], dim=-1)   # bs, (32*4)
        #nlp_bert_token_feat = torch.cat([tit_token_feat, qry_token_feat, des_token_feat, s_att_token_feat], dim=-1)  # bs, (32*4)
