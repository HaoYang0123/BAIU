...
class NLPBertModel(nn.Module):
    def __init__(self, bert_model, device, model_flag="bert",
                 ...
                 feat_hidden_size=768, hidden_size=128, dropout_prob=0.1, max_his_len=205):
        ...
        self.his_sim_fc_item = nn.Sequential(nn.Linear(hidden_size, 1), nn.Tanh())
        ...
        nn.init.xavier_uniform_(self.his_sim_fc_item[0].weight)
    ...
    def forward(self, input_tensor, return_mid_res=False):
        ...
        """
        cur_fea_tensor, cur_des_fea_tensor, cur_des_msk_tensor, his_msk_tensor, his_bro_msk_tensor, his_add_msk_tensor, his_buy_msk_tensor, his_sim_msk_tensor, \
        ...
        sim_fea_tensor, sim_des_fea_tensor = input_tensor
        ...
        if bro_fea_tensor.shape[-1] == self.feat_hidden_size:
            ...
            sim_fea_tensor = self.title_fc(sim_fea_tensor)
            ...
            sim_des_fea_tensor = self.des_fc(sim_des_fea_tensor)
        ...
        his_sim_att_feat = self.atten_title_des_fea(bs, sim_fea_tensor, sim_des_fea_tensor, his_sim_des_msk_tensor)  # bs, 150, 768
        ...
        his_sim_att_feat = self.add_pos_emb(bs, his_sim_att_feat, self.start_sim_pos)
        ...
        his_sim_att_item_feat = self.attention_by_weight(his_sim_att_feat, his_sim_msk_tensor, self.his_sim_fc_item)  # bs, 768
        ...
        #user_stack_feat = torch.stack([his_bro_att_item_feat, his_add_att_item_feat, his_buy_att_item_feat, his_sim_att_item_feat], dim=1)  # bs, 4, 768
        user_stack_feat = torch.stack([his_bro_att_item_feat, his_add_att_item_feat, his_buy_att_item_feat], dim=1)  # bs, 3, 768
        ...
        # his_prob, usr_att_feat = self.attention_score_by_weight(user_stack_feat, his_msk_tensor, self.his_fc)
        his_prob, usr_att_feat = self.attention_score_by_weight(user_stack_feat, his_msk_tensor[:, :3], self.his_fc)
        usr_sim_feat = his_sim_att_item_feat
        # his_prob = None
        ...
        pred_prob = nn.Sigmoid()(torch.sum(itm_feat * usr_feat, dim=-1))  # bs
        pred_prob_sim = nn.Sigmoid()(torch.sum(itm_feat * usr_sim_feat, dim=-1))
        if return_mid_res:
            return pred_prob, pred_prob_sim, his_prob, usr_feat, itm_feat
        return pred_prob, pred_prob_sim
