...
class DeepFM(BaseModel):
    ...
    def __init__(self,
        ...
        self.nlp_linear = nn.Linear(nlp_dim*3, 1, bias=False).to(device)
    ...
    def forward(self, X_linear_feat, tit_feat, qry_feat, des_feat, s_tit_fea, s_des_fea, s_qry_fea, s_tit_des_msk, s_itm_msk):
        # _, tit_feat = self.get_bert_cls_atten_feat(tit_id, tit_mask)  # bs, 32
        # _, qry_feat = self.get_bert_cls_atten_feat(qry_id, qry_mask)  # bs, 32
        # _, des_feat = self.get_bert_cls_atten_feat(des_id, des_mask)  # bs, 32
        # tit_feat/qry_feat/des_feat shape: bs, 768
        # s_tit_fea/s_des_fea/s_qry_fea shape: bs, 10, 768
        # s_tit_des_msk shape: bs, 10, 2
        # s_itm_msk: bs, 10

        #print("MSK", s_tit_des_msk.shape, s_itm_msk.shape)
        ...
        nlp_bert_feat = torch.cat([tit_feat, qry_feat, des_feat], dim=-1)   # bs, (32*3)
