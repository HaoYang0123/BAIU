...
class BaseDataset(Dataset):
    ...
    def _load_item_npy(self, itemid, npy_folder):
        ...
        one_fea = d['fea']
        try:
            if one_fea.shape[0] != 200: return np.random.random(200)
        except:
            return np.random.random(200)
        return d['fea']
    ...
    def _load_session_npy(self, session_list):
        ...
        for (itemid, titleid, desid, queryid) in session_list:
            ...
            if has_tit_flag or has_des_flag or has_qry_flag:
                ...
                if one_tit_fea is None: one_tit_fea = np.random.random(200)
                if one_des_fea is None: one_des_fea = np.random.random(200)
                if one_qry_fea is None: one_qry_fea = np.random.random(200)
        ...
        for idx in range(self.max_num_in_user_session - len(tit_fea_list)):
            ...
            tit_fea_list.append(np.random.random(200))
            des_fea_list.append(np.random.random(200))
            qry_fea_list.append(np.random.random(200))
            tit_des_mask_list.append([0,0,0])
        return tit_fea_list, des_fea_list, qry_fea_list, tit_des_mask_list, item_mask_list
    ...
    def __getitem__(self, idx):
        ...
        tit_fea = self._load_item_npy(ori_tit_id, self.title_npy_folder)  # 768
        qry_fea = self._load_item_npy(ori_qry_id, self.query_npy_folder)  # 768
        des_fea = self._load_item_npy(ori_des_id, self.des_npy_folder)    # 768
        #print("load item npy", time.time()-start_time)
        s_tit_fea, s_des_fea, s_qry_fea, s_tit_des_msk, s_itm_msk = self._load_session_npy(session_list)
        ...
        #print("----", s_tit_des_msk)
        return x, torch.FloatTensor(tit_fea), torch.FloatTensor(qry_fea), torch.FloatTensor(des_fea), s_tit_fea, s_des_fea, s_qry_fea, s_tit_des_msk, s_itm_msk, y
...
class BaseModel(nn.Module):
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
        ...
        train_loader = DataLoader(
            dataset=train_tensor_data, shuffle=shuffle, batch_size=batch_size, num_workers=6)
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            try:
                with tqdm(enumerate(train_loader), disable=verbose != 1) as t:
                    train_step_idx = 0
                    for _, (x_train, tit_fea, qry_fea, des_fea, s_tit_fea, s_des_fea, s_qry_fea, s_tit_des_msk, s_itm_msk, y_train) in t:
                        ...
                        y_pred = model(x, tit_fea, qry_fea, des_fea, s_tit_fea, s_des_fea, s_qry_fea, s_tit_des_msk, s_itm_msk).squeeze()
    ...
    def predict(self, x, ori_x, batch_size=256):
        ...
        test_loader = DataLoader(
            dataset=tensor_data, shuffle=False, batch_size=batch_size, num_workers=6)
        ...
        with torch.no_grad():
            ...
            for _, (x_test, tit_fea, qry_fea, des_fea, s_tit_fea, s_des_fea, s_qry_fea, s_tit_des_msk, s_itm_msk, y_test) in enumerate(test_loader):
                ...
                y_pred = model(x, tit_fea, qry_fea, des_fea, s_tit_fea, s_des_fea, s_qry_fea, s_tit_des_msk, s_itm_msk).cpu().data.numpy()  # .squeeze()
