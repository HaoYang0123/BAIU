...
class WDL(BaseModel):
    ...
    def forward(self, X_linear_feat, tit_feat, qry_feat, des_feat, tit_tok_fea, tit_msk_fea, qry_tok_fea, qry_msk_fea, des_tok_fea, des_msk_fea, s_tit_fea, s_des_fea, s_qry_fea, s_tit_des_msk, s_itm_msk, s_tit_tok, s_tit_msk, s_des_tok, s_des_msk, s_qry_tok, s_qry_msk, tit_tok_score, des_tok_score, qry_tok_score):
        #print("token score shape", tit_tok_score.shape, des_tok_score.shape, qry_tok_score.shape)
        #print("token score", tit_tok_score, des_tok_score, qry_tok_score)
        bs = tit_feat.shape[0]
        #print("BS", bs)
        tit_feat = self.title_fc(tit_feat)  # bs, 32
        qry_feat = self.title_fc(qry_feat)  # bs, 32
        des_feat = self.title_fc(des_feat)  # bs, 32
        ...
        logit += self.nlp_token_linear(nlp_bert_token_feat)
        logit += self.nlp_token_score_linear(tok_score_emb)
